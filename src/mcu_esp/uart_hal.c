/**
 * uart_hal.c - UART bridge to STM32 Klipper MCU
 *
 * Uses ESP-IDF UART driver with a ring buffer.
 * A dedicated task reads from the driver event queue and fires rx_cb.
 */
...
static uart_hal_rx_cb_t s_rx_cb;
...
esp_err_t uart_hal_init(uart_hal_rx_cb_t rx_cb) {
    s_rx_cb = rx_cb;
    ...
    if (xTaskCreate(uart_rx_task, "uart_rx", 3072, NULL, 10, NULL) != pdPASS)
        return ESP_FAIL;
...
esp_err_t uart_hal_send(const uint8_t *data, size_t len) {
