/**
 * spi_slave_hal.c - SPI slave hardware abstraction for host ESP32-C5
 *
...
static const char *TAG = "spi_slave_hal";
...
esp_err_t spi_slave_hal_init(void) {
    esp_err_t ret;
    ...
    BaseType_t rc = xTaskCreatePinnedToCore(
        spi_slave_task, "spi_slave", 4096, NULL, 10, NULL, 1);
    if (rc != pdPASS) {
...
esp_err_t spi_slave_hal_send(const uint8_t *frame) {
...
esp_err_t spi_slave_hal_recv(uint8_t *frame, uint32_t timeout_ms) {
    uint8_t *ptr = NULL;
...
bool spi_slave_hal_tx_ready(void) {
    return uxQueueSpacesAvailable(s_tx_queue) > 0;
}

int spi_slave_hal_rx_pending(void) {
